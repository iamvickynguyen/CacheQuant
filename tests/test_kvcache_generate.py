...
def test_repeated_prompt_hits_maximum_possible_prefix(model_and_tokenizer):
    ...
    text2, _, stats2 = generate_with_prefix_cache(model, tokenizer, prompt, cache, MAX_NEW_TOKENS)
    ...
    # This is the single case with the most cache reuse, so it's the most
    # likely place a K/V shape or position bug would show up if one existed -
    # confirm the generated text is byte-identical to a full recompute, not
    # just that the hit count looks right.
...
def test_eviction_under_pressure_still_produces_correct_output(model_and_tokenizer):
    model, tokenizer = model_and_tokenizer
    # A tiny cap forces some (not all) of prompt_a's cached tokens to be
    # evicted once prompt_b (a disjoint prompt) is inserted: prompt_a is 5
    # tokens, prompt_b is 4, and a cap of 7 evicts exactly 2 of prompt_a's
    # tokens, leaving a genuine partial hit on re-request (not a full miss).
    cache = PrefixKVCache(max_tokens=7)
    prompt_a = "Artificial intelligence began with"
    prompt_b = "The recipe calls for"
    ...
    cached_text, _, stats = generate_with_prefix_cache(model, tokenizer, prompt_a, cache, MAX_NEW_TOKENS)
    baseline_text, _ = generate(model, tokenizer, prompt_a, MAX_NEW_TOKENS)

    assert cached_text == baseline_text
    # Confirm this scenario actually exercises a partial hit (some of
    # prompt_a survived eviction, some didn't) rather than a full miss.
    assert 0 < stats.cached_tokens < stats.prompt_tokens
